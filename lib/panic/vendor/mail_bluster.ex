defmodule Panic.MailBluster do
  @moduledoc """
  ...
  For this to work ensure that in your MailBluster settings the unsubscribe URL is set to https://YOUR_DOMAIN/unsubscribe/mailbluster/%e
  ...
  def sync_all_users_async() do
    if System.get_env("MAIL_BLUSTER_API_KEY") do
      Panic.BackgroundTask.run(fn ->
        Panic.Repo.all(Panic.Accounts.User)
        |> Enum.each(fn user ->
          sync_user(user)
