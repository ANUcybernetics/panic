defmodule Panic.Email do
  ...
  An Swoosh email struct can be delivered by a Swoosh mailer (see mailer.ex & user_notifier.ex). Eg:
      ...
      Panic.Email.confirm_register_email(user.email, url)
      |> Panic.Mailer.deliver()
  """

  use Phoenix.Swoosh,
    view: PanicWeb.EmailView,
    layout: {PanicWeb.EmailView, "email_layout.html"}
  ...
  defp from_name do
    Panic.config(:mailer_default_from_name)
  end

  defp from_email do
    Panic.config(:mailer_default_from_email)
  end
  ...
  def get_unsubscribe_url(user, email_kind) do
    Panic.Accounts.NotificationSubscriptions.unsubscribe_url(user, email_kind)
  end
end
