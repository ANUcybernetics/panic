defmodule PanicWeb.Helpers do
  ...
  def main_menu_items(current_user) do
    PanicWeb.Menus.main_menu_items(current_user)
  end

  def user_menu_items(current_user) do
    PanicWeb.Menus.user_menu_items(current_user)
  end

  def public_menu_items(current_user) do
    PanicWeb.Menus.public_menu_items(current_user)
  end

  def get_menu_item(name, current_user) do
    PanicWeb.Menus.get_link(name, current_user)
  end

  def home_path(nil), do: "/"
  def home_path(current_user), do: PanicWeb.Menus.get_link(:dashboard, current_user).path
