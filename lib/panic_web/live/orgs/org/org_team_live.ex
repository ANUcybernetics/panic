defmodule PanicWeb.OrgTeamLive do
  use PanicWeb, :live_view
  alias Panic.Repo
  alias Panic.Orgs
  import PanicWeb.OrgSettingsLayoutComponent

  on_mount {PanicWeb.OrgOnMountHooks, :require_org_admin}
  ...
  defp apply_action(socket, :invite, _params) do
    socket
    |> assign(:page_title, gettext("Invite new member"))
  end
  ...
  def handle_event("delete_member", %{"id" => id}, socket) do
    ...
    case Orgs.delete_membership(membership) do
      {:ok, membership} ->
        Panic.Logs.log("orgs.delete_member", %{
        ...
        if membership.user_id == socket.assigns.current_user.id do
          {:noreply,
           ...
           |> push_redirect(to: PanicWeb.Helpers.home_path(socket.assigns.current_user))}
        else
          PanicWeb.UserAuth.disconnect_user_liveviews(member_user)
  ...
  def handle_event("delete_invitation", %{"id" => id}, socket) do
    ...
    Panic.Logs.log("orgs.delete_invitation", %{
      user: socket.assigns.current_user,
      org_id: org.id,
      target_user_id: nil,
      metadata: %{
  ...
  def handle_event("close_modal", _, socket) do
    {:noreply,
     push_patch(socket, to: Routes.org_team_path(socket, :index, socket.assigns.current_org.slug))}
  end
