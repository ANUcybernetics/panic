defmodule PanicWeb.NetworkLive.Show do
  ...
  def handle_info({:run_created, %Run{cycle_index: 0} = run}, socket) do
    ...
    socket.assigns.vestaboards |> Enum.each(&Vestaboard.send_text(&1, "blank"))
  ...
  def render(%{live_action: :terminal} = assigns) do
    ~H"""
    <div class="relative p-36 w-screen h-screen bg-black text-purple-300 cursor-none overflow-hidden">
      <.live_component
        module={PanicWeb.NetworkLive.InitialPromptComponent}
        id="initial-prompt"
        network={@network}
