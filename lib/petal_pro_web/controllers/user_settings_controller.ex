  ...
  def update_password(conn, params) do
    ...
    case Accounts.update_user_password(user, password, user_params) do
      {:ok, user} ->
        ...
        |> put_session(:user_return_to, Routes.live_path(conn, PetalProWeb.EditPasswordLive))
        |> UserAuth.log_in_user(user)

      {:error, changeset} ->
        conn
        |> put_flash(:error, PetalProWeb.ErrorHelpers.combine_changeset_error_messages(changeset))
        |> redirect(to: Routes.live_path(conn, PetalProWeb.EditPasswordLive))
    end
  end
  ...
  def confirm_email(conn, %{"token" => token}) do
    case Accounts.update_user_email(conn.assigns.current_user, token) do
      :ok ->
        ...
        |> redirect(to: Routes.live_path(conn, PetalProWeb.EditProfileLive))
