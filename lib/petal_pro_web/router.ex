defmodule PetalProWeb.Router do
  ...
  scope "/", PetalProWeb do
    ...
    live_session :public,
      on_mount: [
        {PetalProWeb.UserOnMountHooks, :maybe_assign_user}
      ] do
    ...
    scope "/org/:org_slug" do
      ...
      live_session :require_org_member,
        on_mount: [
          {PetalProWeb.UserOnMountHooks, :require_confirmed_user},
          {PetalProWeb.OrgOnMountHooks, :assign_org_data},
          {PetalProWeb.OrgOnMountHooks, :require_org_member}
        ] do
  ...
  if Mix.env() in [:dev, :test] do
    scope "/dev" do
      ...
      scope "/emails" do
        ...
        get "/", PetalProWeb.EmailTestingController, :index
        get "/sent", PetalProWeb.EmailTestingController, :sent
        get "/preview/:email_name", PetalProWeb.EmailTestingController, :preview
        post "/send_test_email/:email_name", PetalProWeb.EmailTestingController, :send_test_email
        get "/show/:email_name", PetalProWeb.EmailTestingController, :show_html
      end
    end
  ...
  defp onboard_new_users(conn, _opts) do
    if conn.assigns[:current_user] && !conn.assigns.current_user.is_onboarded do
      ...
      |> redirect(
        to:
          Routes.live_path(conn, PetalProWeb.UserOnboardingLive,
