defmodule PanicWeb.EditPasswordLiveTest do
  ...
  describe "when signed in" do
    ...
    test "event send_password_reset_email", %{conn: conn, user: _user} do
      {:ok, view, _html} = live(conn, Routes.live_path(conn, PanicWeb.EditPasswordLive))
  ...
  describe "when signed out" do
    test "can't access the page", %{conn: conn} do
      live(conn, Routes.live_path(conn, PanicWeb.EditEmailLive))
