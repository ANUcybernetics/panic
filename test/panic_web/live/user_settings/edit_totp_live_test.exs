defmodule PanicWeb.EditTotpLiveTest do
  ...
  describe "Index (2FA)" do
    ...
    test "requires password to enable 2FA", %{conn: conn} do
      {:ok, view, html} = live(conn, Routes.live_path(conn, PanicWeb.EditTotpLive))
    ...
    test "enables totp secret", %{conn: conn, user: user} do
      {:ok, view, _html} = live(conn, Routes.live_path(conn, PanicWeb.EditTotpLive))
