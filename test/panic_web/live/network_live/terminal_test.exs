defmodule PanicWeb.NetworkLive.TerminalTest do
  ...
  describe "anonymous user with QR code token" do
    setup do
      ...
      {:ok, network} =
        network
        |> Ash.Changeset.for_update(:update_models, %{
          models: ["dummy-t2t"]
        }, actor: user)
        |> Ash.update()
    ...
    test "can access terminal with valid token and start run successfully", %{conn: conn, network: network, token: token, user: user} do
      ...
      invocations = Panic.Engine.Invocation |> Ash.read!(actor: user)
