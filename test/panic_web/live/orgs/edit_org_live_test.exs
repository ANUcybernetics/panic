defmodule PanicWeb.EditOrgLiveTest do
  ...
  describe "valid params" do
    test "can edit an org", %{conn: conn, org: org, user: _user} do
      edit_org_path = Routes.live_path(conn, PanicWeb.EditOrgLive, org.slug)
  ...
  describe "invalid params" do
    test "shows errors", %{conn: conn, org: org, user: _user} do
      {:ok, view, _html} = live(conn, Routes.live_path(conn, PanicWeb.EditOrgLive, org.slug))
    ...
    test "it redirects", %{conn: conn, org: _org, user: user} do
      ...
      assert {:error,
              {:redirect,
               ...
               live(conn, Routes.live_path(conn, PanicWeb.EditOrgLive, new_org.slug))
    end
  end
end
